...
static NSString * const ktest_longpress_pinchJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_test_longpress_pinchInjection
{
	...
	[html appendString:ktest_longpress_pinchJavascript];
...
{
	// attempt to make well-formed HTML and inject in our test_longpress_pinch bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _test_longpress_pinchInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to test_longpress_pinch support.",url);
				}
			}
